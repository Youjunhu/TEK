TEK performs delta-f PIC simulation of low-frequency electromagnetic turbulence in tokamak plasmas using the gyrokinetic model for all species (with electrons in the zero Larmor radius limit)
...
Input to TEK: a fortran namelist `inmput.nmlt`, which specify various numerical parameters (e.g. grid point number), filenames of equilibrium magnetic configuration (g-eqdsk file), and species radial profiles (e.g. number density and temperature).
