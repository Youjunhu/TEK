...
module control_parameters
  ...
  logical  :: filter_toroidal, filter_radial, diagnosis !if ture, output additional testing information
  integer  :: order_faraday_scheme
  integer :: space_charge_switch, fk_switch
  integer :: ismooth
  !integer :: nh !toroidal harmonics included are exp(i*nsegment*y) with i=(-nh, -nh+1,..., -1,0,1,2,...,nh-1,nh)
  !harmonics included are exp(i*nsegment*y) with i = nh_min, ..., nh_max and i = -nh_max, ..., -nh_min
  integer :: nh_min !non-negative value, usually zero
  integer :: nh_max !non-negative value
...
contains
  ...
  subroutine allocate_field_matrix()
    ...
    if(myid.eq.0) write(*,*) 'mtor, nrad=', m, n
