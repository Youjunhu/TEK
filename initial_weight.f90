module delta_ne_mod
  ...
  use gk_module,only:ngk0 !1/m^3
  use magnetic_coordinates,only: xlow,xupp, ntor=>nsegment
...
contains
  function delta_ne(radcor,theta,alpha) result (z) ! 1/m^3
    ...
    integer:: i
    delta_ne0=ngk0(1)*1.d-4 !amplitude of the initial perturbation in terms of the equilibrium density
    !kradcor0=twopi/(xupp-xlow) !radial wave number
    kradcor0=pi/(xupp-xlow) !the fundament radial wave number in sine expansion
    radcor_center=(xupp+xlow)/two
    radcor_width=(xupp-xlow)*0.25_p_
    q=qfunc(radcor)
    ...
    integer_val=ntor*nh_max*nint((qmax+qmin)/2)
    mprime=ntor*nh_max*q-integer_val
    ...
    !z=delta_ne0*sin(kradcor0*(radcor-xlow))*cos(mprime*theta+ntor*alpha) !for ITG instabilities
    ...
    !z=delta_ne0 !for testing
    z=0._p_
    do i=0,nh_max !set exponential radial dependence so that the perturbation near the radial boundary is small
  ...
  function delta_ne_theta_orgin(radcor,theta,alpha) result (z) ! temporary, for testing
    real(p_)::radcor,theta,alpha,z
    delta_ne0=ngk0(1)*0.01_p_ !amplitude of the initial perturbation is chosen 0.05 of the equilibrium density
    kradcor0=twopi/(xupp-xlow) !radial wave number
    radcor_center=(xupp+xlow)/two
    radcor_width=(xupp-xlow)*0.1_p_
    q=qfunc(radcor)
    !mprime=mod(ntor*q,1.0_p_) !this value is chosen in order to satisfy perioidic condition on (theta,alpha) plane when using the following spatial distribution
    qmax=qfunc(xupp)
    qmin=qfunc(xlow)
    ...
    z=-delta_ne0*sin(kradcor0*(radcor-xlow))*sin(mprime*theta+ntor*alpha)*mprime
  ...
  function delta_ne_psi(radcor,theta,alpha) result (z) ! temporary, for testing
    ...
    !z=kradcor0*delta_ne0*cos(kradcor0*(radcor-xlow))*cos(mprime*theta+ntor*alpha) !wrong, there is a radial dependence in mprime
    del_psi=0.01*(xupp-xlow)
  ...
  function delta_ne_alpha(radcor,theta,alpha) result (z) ! temporary, for testing
    ...
    z=-delta_ne0*sin(kradcor0*(radcor-xlow))*sin(mprime*theta+ntor*alpha)*ntor
...
!!$endif
  ...
  function initial_delta_f_i(radcor,theta,alpha,v) result (z) ! for tesing Monte-Carlo integration, v in unit of vn, z in unit 1/(Ln**3*vn**3)
    ...
    use fk_module,only: vn_fk, mass_i,ti0
  ...
  subroutine initialize_gk_weight()
    ...
    use gk_module,only: nsm, nm_gk, vn_gk,ngk0, ptcl_num0_gk, &
                       mu_gk, vpar_gk,xgc,zgc,ygc, w_gk !output
    use magnetic_coordinates,only:mpol,nrad,xgrid,zgrid, nsegment
    use control_parameters, only : nh_max
...
do ns=1,nsm
    do k=1,nm_gk(ns)
       !call linear_2d_interpolate(mpol,nrad,zgrid,xgrid,b_mc,zgc(k),xgc(k),b_val)
       !v=sqrt(two*mu_gk(k)*b_val*2+vpar_gk(k)**2) !in unit of vn_gk, wrong! a bug found when tested the deposition procedure
       ! v=sqrt(mu_gk(k)*two*b_val+vpar_gk(k)**2) !in unit of vn_gk
       !     w_gk(k)=ps_vol_gk(k)*(mu_gk(k)*b_val*2+vpar_gk(k)**2)/3.0*fe2_arbitrary(xgc(k),zgc(k),ygc(k),v) !initial weight of markers, for testing
       w_gk(k,ns)=ptcl_num0_gk(k,ns)*0.001*cos(ygc(k,ns)*nsegment*nh_max)
       !call sub_random_yj(0, next_seed, rannum) !0 means using last random number as iseed        
       !w_gk(k,ns) = ptcl_num0_gk(k,ns)*(rannum-0.5)*0.001
       !write(*,*) 'w_gk=',w_gk(k),ps_vol_gk(k),initial_delta_f_e(xgc(k),zgc(k),ygc(k)
    enddo
 enddo
  ...
  function initial_delta_f_e(radcor,theta,alpha,v,vpar,ns) result (z) ! v in unit of vn_gk, z in unit 1/(Ln**3*vn**3)
    use constants,only: p_
    use constants,only: two,twopi,kev
    use normalizing,only: Ln
    use gk_module,only: mass_gk,tgk0, vn_gk
    use delta_ne_mod,only: delta_ne
    implicit none
    integer,intent(in) :: ns
    real(p_),intent(in) :: radcor,theta,alpha,v,vpar
    real(p_) :: z
    real(p_),parameter:: vpar_drift=0. !in unit of vn_gk
    real(p_):: vper_sq

    vper_sq=v**2-vpar**2
    z=delta_ne(radcor,theta,alpha)*sqrt((mass_gk(ns)/(twopi*tgk0(ns)*kev))**3) &
         & *exp(-mass_gk(ns)*(vper_sq*vn_gk(ns)**2+(vpar-vpar_drift)**2*vn_gk(ns)**2)/(two*tgk0(ns)*kev))
    z=z*(vn_gk(ns)**3*Ln**3)
