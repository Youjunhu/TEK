...
program main
  ...
  mpar = numprocs/ntube !poloidal grids for perturbed field
  if(mod(mpol-1, mpar) .ne. 0) then
     write(*,*) 'Error: Mod(mpol-1, numprocs/ntube) must be zero', &
          & 'Please adjust poloidal gridpoint number mpol (in the input namelist) or numprocs'
     goto 1234 !end the job
  endif
