...
fftw_include=-I/global/homes/y/yjhu/installed/fftw-3.3.7/include
...
#petsc_included:=-I/global/u2/y/yjhu/installed/petsc-3.9.3_cori/include -I/global/u2/y/yjhu/installed/petsc-3.9.3_cori/cori_yj/include
   ...
   OPTION= -O3
   fftw_include=-I/THL8/software/fftw/3.3.4-icc16-IMPI5.1/include/
  ...
  blas_location:=  /THL6/software/lapack/3.8.0-icc16/lib64/libblas.a
     ...
     COMPILER=	mpif90
    fftw_include=-I/thfs3/software/fftw/3.3.8-gcc9.3.0-mpi-x/include/
...
else ifeq (${which_machine}, sm)
  COMPILER=	mpif90
  OPTION= -O3 -lfftw3 
  fftw_include=-I/public/fuyun/software/FFTW/3.3.10-GCC-13.2.0/include/
  lib2 = -lopenblas

else ifeq (${which_machine}, thex)
   COMPILER=	mpif90
...
# blas_location:=  /fs2/home/huyj/install/lapack-3.12.1/libblas.a
lapack_location:= /fs2/software/lapack/3.10.0-icc19.0/lib/liblapack.a
blas_location:= /fs2/software/lapack/3.10.0-icc19.0/lib/libblas.a
...
f90source = modules.f90 pputil_yj.f90  splines.f90  math.f90  \
                equilibrium.f90 magnetic_coordinates.f90  mapping.f90 \
...
%.o: %.f90 modules.f90
	$(COMPILE) $(fftw_include) $< -o $@

.PHONY : clean run tarfile create_tags to_cori to_edison on_tianhe
clean :
	 rm -f $(program_name)   $(f90objs) $(f77objs) *.mod *.pd ms/*

create_tags:
	ctags-exuberant -e  $(f90source) $(f77source)
...
run: $(program_name)
	date +%H:%M:%S
	export OMP_NUM_THREADS=1 && mpiexec -n 16 ./$(program_name) 
	date +%H:%M:%S
on_tianhe:
	export which_machine=tianhe; make
on_thex:
	echo "on thex machine"
	export which_machine=thex; make
on_sm:
...
to_hfc:
	make clean && rsync -avz --delete --exclude=".git" -e "ssh -p 65062  -i ~/Downloads/hfcas_user26_hfeshell.nscc-hf.cn_RsaKey.txt" ./ hfcas_user26@hfeshell.nscc-hf.cn:~/tek/
